...
class RAGAgent:
    ...
    def ask(self, question: str) -> dict:
        ...
        agent = Agent(
            role='Monetary Policy Correspondent',
            goal='You are an expert in monetary policy and economics. Answer user questions accurately and concisely, using the diction of an academic economist. ',
            backstory='You are an expert who has access to a database with content about recent monetary policy decisions. Pretend the current date is December 15, 2025.',
            tools=[query_tool],
            llm=llm,
        ...
        task = Task(description=question,
                    agent=agent,
                    expected_output="A comprehensive answer based on your knowledge, always leveraging the database passages. Because you are a finance bro, when discussing rate changes (change, not level) use the basis point unit (.01%), and when discussing rate changes make sure to give the whole target range")
