...
with st.sidebar:
    ...
    top_k = st.slider(value=config.DEFAULT_TOP_K,
                      min_value=1,
                      max_value=20,
                      step=1,
                      label="Top K Passages",
                      help="Number of top similar passages to retrieve from the database.") 
    ...
    st.markdown("""
    ### About
    This RAG assistant answers questions about recent happenings in Monetary Policy based upon:
    - Monetary policy presss releases
    """)

# -----------------------------------------------------------------------------
# Main App Header
# -----------------------------------------------------------------------------
st.title("💵 Monetary Policy RAG Assistant")
st.markdown("Ask questions about recent happenings in Monetary Policy and get AI-powered answers based on curated content.")
...
# We'll accept input from either the chat input box OR from example buttons.
# If an example button is pressed we set `st.session_state.pending_prompt`.

# Determine the active prompt: chat input takes priority, otherwise use pending example
user_prompt = None
if prompt := st.chat_input("Ask a question about recent occurences in Monetary Policy..."):
    user_prompt = prompt
...
with st.expander("💡 Example Questions"):
    examples = [
        "Who is the current Chair of the FOMC?",
        "What are the current FOMC voting members?",
        "What recent changes have been made to monetary policy?",
        "What is the structure of Federal Reserve press releases?",
        "What are the key variables the Fed looks at to make monetary policy decisions?",
        "What is the Fed signaling it will do with interest rates in the first half of 2026?",
        "What does the Fed believe the Central Bank of Mars will do next year with the Intergalactic Interest Rate?"
    ]
    
    for example in examples:
        if st.button(example, key=example):
            # Set the example as a pending prompt so it will be handled
